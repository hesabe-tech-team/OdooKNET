...
class PaymentAcquirerHesabe(models.Model):
    ...
    provider = fields.Selection(selection_add=[('Hesabe_PG_KNET_v15', 'Hesabe KNET')], ondelete={'Hesabe_PG_KNET_v15': 'set default'})
    merchant_code = fields.Char(string="Merchant Code", help="This is Merchant Code", required_if_provider='Hesabe_PG_KNET_v15')
    secret_key = fields.Char(string="Secret Key", help="This is Secret Key", required_if_provider='Hesabe_PG_KNET_v15')
    access_code = fields.Char(string="Access Code", help="This is Access Code", required_if_provider='Hesabe_PG_KNET_v15')
    iv_key = fields.Char(string="IV Key", help="This is IV Key", required_if_provider='Hesabe_PG_KNET_v15')
    api_version = fields.Char(string="API Version", help="This is API Version", required_if_provider='Hesabe_PG_KNET_v15', default='2.0')
    production_url = fields.Char(string="Production Url", help="This is Production Url", required_if_provider='Hesabe_PG_KNET_v15')
    sandbox_url = fields.Char(string="Sandbox Url", help="This is Sandbox Url", required_if_provider='Hesabe_PG_KNET_v15')
    ...
    def _get_hesabe_form_generate_values(self, acqid,currencyid,partnerid,referenceno,providr,amount):
        ...
        payload = {
            ...
            "amount": amount,
            "responseUrl": urls.url_join(base_url, '/payment/hesabe/%s/return' % ('knet' if self.provider == 'Hesabe_PG_KNET_v15' else 'mpgs')),
            "paymentType": 1 if self.provider == 'Hesabe_PG_KNET_v15' else 2,
            "version": acqid.api_version,
            "orderReferenceNumber": referenceno,
            "failureUrl": urls.url_join(base_url, '/payment/hesabe/%s/fail' % ('knet' if self.provider == 'Hesabe_PG_KNET_v15' else 'mpgs')),
    ...
    def _get_default_payment_method_id(self):
        self.ensure_one()
        if self.provider != 'Hesabe_PG_KNET_v15':
          return super()._get_default_payment_method_id()
        return self.env.ref('Hesabe_PG_KNET_v15.payment_method_Hesabe_PG_KNET_v15').id