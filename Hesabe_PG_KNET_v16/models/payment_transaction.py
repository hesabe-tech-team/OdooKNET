...
class PaymentTransactionHesabe(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        if self.provider_code != 'Hesabe_PG_KNET_v16':
            return res
    ...
    def _get_tx_from_notification_data(self, provider_code, data):
        ...
        if provider_code != 'Hesabe_PG_KNET_v16' or len(transaction) == 1:
            return transaction
            
        reference = data.get('response').get('orderReferenceNumber')
        transaction = self.search([('reference', '=', reference), ('provider_code', '=', 'Hesabe_PG_KNET_v16')])
        if not transaction:
            error_msg = (_(
